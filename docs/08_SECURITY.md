...
## 2026-09 UI/security upgrade

The biometric App Lock path now uses an Android Keystore AES credential with
per-use user authentication and biometric-enrollment invalidation. The
BiometricPrompt uses `BIOMETRIC_STRONG` with a `CryptoObject`, so successful
unlock is tied to the Keystore credential rather than only receiving a
