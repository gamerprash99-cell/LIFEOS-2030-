> **CURRENT IMPLEMENTATION — 2026-09-11:** LifeOS uses Kotlin + Jetpack Compose + Room + a hand-written `ServiceLocator` + Compose Navigation. The Intelligence Engine is on-device and offline; there is no external AI API, API key, Firebase, telemetry, or `INTERNET` permission in the current app. Older provider/network descriptions elsewhere in historical documentation are not the current implementation.
