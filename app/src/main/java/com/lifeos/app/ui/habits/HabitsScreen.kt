...
class HabitsViewModel(private val habitRepository: HabitRepository) : ViewModel() {
    ...
    init {
        viewModelScope.launch {
            habits.collectLatest { current ->
                _analytics.value = current.associate { habit ->
                    habit.id to habitRepository.computeAnalytics(habit)
                }
            }
        }
    }
            ...
            Column(modifier = Modifier.weight(1f)) {
                ...
                analytics?.let { stats ->
                    Row(
                        ...
                        Text("🔥 ${stats.currentStreak}", style = MaterialTheme.typography.labelSmall)
                        Text("🏆 ${stats.longestStreak}", style = MaterialTheme.typography.labelSmall)
                        Text("7d ${stats.completionPercentThisWeek}%", style = MaterialTheme.typography.labelSmall)
                        Text("30d ${stats.completionPercentThisMonth}%", style = MaterialTheme.typography.labelSmall)
                    }
                }
            }
