    ...
    NavigationBar(modifier = Modifier.fillMaxWidth(), containerColor = MaterialTheme.colorScheme.surface.copy(alpha = .96f), tonalElevation = 0.dp, windowInsets = WindowInsets.navigationBars) {
        items.forEach { item ->
            ...
            NavigationBarItem(
                selected = selected,
                onClick = {
                    if (item.screen == Screen.Home) navController.popBackStack(Screen.Home.route, inclusive = false)
                    else navController.navigate(item.screen.route) { popUpTo(Screen.Home.route) { saveState = true }; launchSingleTop = true; restoreState = true }
                },
                icon = { Icon(if (selected) item.selectedIcon else item.unselectedIcon, contentDescription = item.label) },
                label = { Text(item.label) },
                colors = NavigationBarItemDefaults.colors(selectedIconColor = LifeOSPrimary, selectedTextColor = MaterialTheme.colorScheme.onSurface, indicatorColor = LifeOSLavender.copy(alpha = .72f), unselectedIconColor = MaterialTheme.colorScheme.onSurfaceVariant, unselectedTextColor = MaterialTheme.colorScheme.onSurfaceVariant)
            )
        }
    }
}
