...
class TaskRepository(private val dao:TaskDao,private val appContext:Context){
 ...
 suspend fun createTask(title:String,description:String?=null,dueDateEpochDay:Long?=null,dueTimeMinutes:Int?=null,priority:TaskPriority=TaskPriority.MEDIUM,category:String?=null,reminderEpochMillis:Long?=null,repeatRule:RepeatRule=RepeatRule.NONE,repeatDaysCsv:String?=null,sourceType:String?=null,sourceId:String?=null):String{val id=IdGenerator.newId();val now=System.currentTimeMillis();dao.upsert(TaskEntity(id=id,title=title,description=description,dueDateEpochDay=dueDateEpochDay,dueTimeMinutes=dueTimeMinutes,priority=priority,category=category,reminderEpochMillis=reminderEpochMillis,repeatRule=repeatRule,repeatDaysCsv=repeatDaysCsv,sourceType=sourceType,sourceId=sourceId,createdAt=now,updatedAt=now));if(reminderEpochMillis!=null&&reminderEpochMillis>System.currentTimeMillis())ReminderScheduler.scheduleTaskReminder(appContext,id,reminderEpochMillis);return id}
 suspend fun createFromAiExtraction(titles:List<String>,dueDateEpochDay:Long?,sourceType:String,sourceId:String):List<String>=titles.map{createTask(title=it,dueDateEpochDay=dueDateEpochDay,sourceType=sourceType,sourceId=sourceId)}
