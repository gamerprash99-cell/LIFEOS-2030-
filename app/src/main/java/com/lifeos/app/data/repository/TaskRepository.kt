...
class TaskRepository(private val dao: TaskDao, private val appContext: Context) {
    ...
    suspend fun createTask(title: String, description: String? = null, dueDateEpochDay: Long? = null, dueTimeMinutes: Int? = null, priority: TaskPriority = TaskPriority.MEDIUM, category: String? = null, reminderEpochMillis: Long? = null, repeatRule: RepeatRule = RepeatRule.NONE, repeatDaysCsv: String? = null, sourceType: String? = null, sourceId: String? = null): String {
        val id = IdGenerator.newId(); val now = System.currentTimeMillis()
        dao.upsert(TaskEntity(id, title, description, dueDateEpochDay, dueTimeMinutes, priority, category, reminderEpochMillis, repeatRule, repeatDaysCsv, false, null, false, null, "[]", sourceType, sourceId, now, now))
    ...
    suspend fun createFromAiExtraction(titles: List<String>, dueDateEpochDay: Long?, sourceType: String, sourceId: String): List<String> = titles.map { createTask(it, dueDateEpochDay = dueDateEpochDay, sourceType = sourceType, sourceId = sourceId) }
    ...
    suspend fun search(query: String): List<TaskEntity> = if (query.isBlank()) emptyList() else dao.search(query)
