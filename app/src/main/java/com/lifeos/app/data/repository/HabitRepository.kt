...
class HabitRepository(
    ...
    suspend fun computeAnalytics(habit: HabitEntity, today: LocalDate = DateTimeUtils.today()): HabitAnalytics {
        ...
        val weekCompletions = allCompletions.filter {
            it.dateEpochDay in weekStart.toEpochDay()..minOf(today.toEpochDay(), weekEnd.toEpochDay())
        }
        ...
        val daysElapsedThisWeek = (today.toEpochDay() - weekStart.toEpochDay() + 1).toInt()
        val weekDoneCount = weekCompletions.count { it.progressCount >= habit.goalCount }
        val weekPercent = if (daysElapsedThisWeek > 0) (weekDoneCount * 100) / daysElapsedThisWeek else 0
        ...
        return HabitAnalytics(
            ...
            completionsThisWeek = weekDoneCount,
            totalDaysThisWeek = daysElapsedThisWeek,
            completionPercentThisWeek = weekPercent,
