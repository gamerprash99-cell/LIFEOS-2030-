...
class NoteRepository(private val dao: NoteDao) {
    ...
    suspend fun createNote(title: String, blocks: List<NoteBlock>, folder: String? = null, tags: List<String> = emptyList()): String {
        ...
        dao.upsert(NoteEntity(id=id,title=title,contentJson=json.encodeToString(blocks),plainTextForSearch=flattenBlocks(blocks),folder=folder,tagsCsv=tags.joinToString(","),createdAt=now,updatedAt=now))
        return id
    }
    suspend fun updateNoteContent(id: String, title: String, blocks: List<NoteBlock>) { val existing=dao.getById(id)?:return; dao.update(existing.copy(title=title,contentJson=json.encodeToString(blocks),plainTextForSearch=flattenBlocks(blocks),updatedAt=System.currentTimeMillis())) }
    suspend fun setFolder(id: String, folder: String?) { val existing=dao.getById(id)?:return; dao.update(existing.copy(folder=folder,updatedAt=System.currentTimeMillis())) }
    suspend fun setTags(id: String, tags: List<String>) { val existing=dao.getById(id)?:return; dao.update(existing.copy(tagsCsv=tags.joinToString(","),updatedAt=System.currentTimeMillis())) }
